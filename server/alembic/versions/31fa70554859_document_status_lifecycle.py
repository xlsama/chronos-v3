...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE project_documents SET status = 'error', error_message = 'Interrupted during previous indexing' WHERE status = 'processing'"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE project_documents SET status = 'processing' WHERE status IN ('pending', 'indexing', 'error')"
    )
