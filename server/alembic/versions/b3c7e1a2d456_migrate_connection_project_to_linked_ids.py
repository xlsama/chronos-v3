...
def upgrade() -> None:
    ...
    op.add_column(
        "projects",
        sa.Column("linked_connection_ids", JSONB, server_default=sa.text("'[]'"), nullable=False),
    )
    ...
    rows = conn.execute(
        sa.text("SELECT id, project_id FROM connections WHERE project_id IS NOT NULL")
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_connections_project_name", "connections", ["project_id", "name"]
    )
