...
- 类型：`flowchart TD`（从上到下）
- **不要使用 subgraph**，Server 归属写在节点标签内（用 `\n` 换行）
- 节点 ID 为 Service 名称，标签格式 `"emoji Service名 :端口\nServer名"`
- 按服务类型选择节点形状：
...
3. **保持现有内容不变**，只增量补充新信息
4. 新发现的 Server/Service 关系要同步更新表格和架构图：新 Server → 加表格行，新 Service → 加表格行 + 加节点（选合适的形状和 emoji），新依赖 → 加箭头
