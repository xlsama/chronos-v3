...
async def investigation_missing_explanation_node(state: InvestigationState) -> dict:
    """当需要审批的工具调用缺少 explanation 时，用 ToolMessage 要求 LLM 重新调用。"""
    sid = state["incident_id"][:8]
    log = get_logger(component="inv_missing_explanation", sid=sid)
    last_message = state["messages"][-1]
    tool_messages = []
    if hasattr(last_message, "tool_calls") and last_message.tool_calls:
        for tc in last_message.tool_calls:
