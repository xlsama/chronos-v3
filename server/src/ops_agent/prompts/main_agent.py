...
2. **定位服务**: 从知识库上下文获取 AGENTS.md 中的服务器和服务信息，找到涉及的服务及所在服务器。知识库信息视为参考假设，排查时通过实际命令验证；如果信息不足，可在排查开始时各调用一次 list_servers 和 list_services 获取完整的环境信息（它们一次即返回全部结果），遇到瓶颈再 ask_human。
...
- **list_servers()**: 列出所有可用服务器，一次返回全部结果。知识库已提供明确服务器信息时无需调用。
...
4. **信息不足时补关键缺口**: 可先基于已有上下文排查；当缺少关键信息导致无法继续时，调用 ask_human 工具精确提问。question 只写关键问题（1-3行），推理分析保留在思考中。
