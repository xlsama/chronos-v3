...
6. **首轮探测不要吞掉真实错误**: 第一次验证失败时，不要用 `2>/dev/null`、`>/dev/null`、`curl -s`、`<命令> || echo "..."` 或会掩盖左侧失败的管道写法把原始错误藏掉。需要压制进度条时优先用 `curl -sS`，并保留 stderr。
...
7. **Docker First 是默认首选**: 当进入运行面排查时，`docker ps -a 2>&1` 应作为第一条探测命令。大多数生产服务都是容器化部署的，先探测 Docker 的成本极低（一条命令），但跳过它的代价极高（可能导致长时间无效探索）。只有当 Docker 探测明确返回 `docker: command not found`（PATH 中没有 docker）时，才跳过 Docker 转入其他观测面。如果返回的是权限错误，按原则 12 走 sudo 重试。
