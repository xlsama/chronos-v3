...
6. **首轮探测不要吞掉真实错误**: 第一次验证失败时，不要用 `2>/dev/null`、`>/dev/null`、`curl -s`、或会掩盖左侧失败的管道写法把原始错误藏掉。需要压制进度条时优先用 `curl -sS`，并保留 stderr。
   - **反面示例**: `docker ps -a 2>/dev/null || echo "Docker not available"` — 这会把 permission denied 吞掉，导致你无法触发 sudo 重试
   - **正确写法**: `docker ps -a 2>&1` — 保留完整 stderr，才能判断是权限问题还是 Docker 真的不存在
