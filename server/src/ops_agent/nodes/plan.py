...
@retry(
    ...
    retry=retry_if_exception_type(
        (TimeoutError, ConnectionError, OSError, RateLimitError, APIConnectionError, APITimeoutError)
    ),
...
async def _stream_plan_llm(llm, messages, publisher, channel, log):
    """Stream plan LLM with retry on transient errors."""
    content = ""
...
async def plan_node(state: MainState) -> dict:
    ...
    # 发布 plan_started 事件，让前端立即切换到 planning 阶段
    ...
    try:
        ...
        log.warning("Failed to publish plan_progress event", error=str(e))

    content = await _stream_plan_llm(llm, messages, publisher, channel, log)
    try:
