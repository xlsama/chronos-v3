...
| Prometheus | PromQL 表达式字符串 | `up{job="prometheus"}` |
...
**Metadata Lock 排查**（DDL 阻塞场景）：
```sql
-- 查看等待 metadata lock 的会话
...
| Metadata Lock 死锁 | `SHOW PROCESSLIST` 筛选 `Waiting for table metadata lock` + `SHOW OPEN TABLES` 确认 `In_use` + 定位长时间 `Sleep` 连接 |
...
### Metadata Lock 死锁专项处理

**症状识别**：DDL 操作（如 `OPTIMIZE TABLE`、`ALTER TABLE`）无响应，同时大量查询堆积，状态显示 `Waiting for table metadata lock`

**排查步骤**：
1. 确认症状：`SHOW PROCESSLIST` 发现多个会话处于 `Waiting for table metadata lock`
2. 定位锁持有者：查找 `command='Sleep'` 且 `time` 值大的空闲连接（通常由连接池未正确释放导致）
3. 确认锁对象：`SHOW OPEN TABLES WHERE In_use > 0` 验证目标表被占用
4. 决策修复：评估后优先 `KILL` 长时间非业务连接（避免终止活跃事务）
...
**预防措施**：
- 连接池配置：设置合理的 `wait_timeout` 和 `interactive_timeout`（建议 600-1800 秒）
- DDL 维护窗口：将结构变更操作放在低峰期执行
- 监控告警：`performance_schema.metadata_locks` 监控 metadata lock 等待时长
...
- **Metadata Lock 特殊注意**：MySQL 中 DDL 操作与空闲连接形成的死锁不会出现在 `INNODB_TRX`，需通过 `SHOW PROCESSLIST` 的状态字段识别