...
export function PlanContent() {
  ...
  const thinkingContent = useIncidentStreamStore((s) => s.planThinkingContent);
  const planningStatus = useIncidentStreamStore((s) => s.phaseState.planning);
  const planProgress = useIncidentStreamStore((s) => s.planProgress);
  ...
  if (!planMd && !thinkingContent) {
    if (planningStatus === "active") {
      const progressText =
        planProgress === "first_token_received"
          ? "已连接，正在生成调查计划"
          : planProgress === "llm_call_started"
            ? "正在制定计划"
            : "正在分析问题，生成调查计划";
      return (
        <div className="py-2">
